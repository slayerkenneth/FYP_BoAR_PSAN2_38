    ...
    public override void skill(GameObject player) {
        ...
        foreach (GameObject enemy in enemyList)
        {
            ...
            if (dir.magnitude < freezeDistance && InBetween(player.transform.rotation.eulerAngles.y, freezeAngle, angle))
            {
                ...
                var temp = enemyPathfinding.walkingSpeed;
                enemyPathfinding.walkingSpeed = 0;
                Task.Delay(Convert.ToInt32(freezeTime[lv]*1000)).ContinueWith(t => { enemyPathfinding.walkingSpeed = temp; });
            }
        }
    }
